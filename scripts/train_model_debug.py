# ✅ scripts/train_model_debug.py (revisión y validación de entrenamiento)
...
dataset["train"] = dataset["train"].map(prepare, remove_columns=dataset["train"].column_names, num_proc=4)
dataset["test"] = dataset["test"].map(prepare, remove_columns=dataset["test"].column_names, num_proc=2)
gc.collect()
...
training_args = TrainingArguments(
    ...
    eval_steps=250,                   # ✅ Eval cada 250 pasos
    save_strategy="steps",
    save_steps=250,                   # ✅ Checkpoint cada 250 pasos
    save_total_limit=5,
    learning_rate=3e-4,
    warmup_steps=500,
    num_train_epochs=10,
...
# === Entrenamiento
print("[🔥] Entrenando para validar estabilidad...")
