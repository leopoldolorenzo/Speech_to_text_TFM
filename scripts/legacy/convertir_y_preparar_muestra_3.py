# convertir_y_preparar_muestra_3.py
# Convierte una muestra de 150.000 archivos nuevos .mp3 a .wav 16kHz mono,
# evitando repetir los usados en fine_tune_01 y fine_tune_02. Genera TSV limpio con transcripciones.
...
CLIPS_WAV = os.path.join(DIR_BASE, "fine_tune_03", "clips_wav")
TSV_ORIG = os.path.join(DIR_BASE, "validated.tsv")
TSV_OUT = os.path.join(DIR_BASE, "fine_tune_03", "dataset_150k.tsv")
...
usadas = set()
for path in [USED_LIST_1, USED_LIST_2]:
...
with open(TSV_ORIG, "r", encoding="utf-8") as fin, \
     ...
     open(USED_LIST_3, "w", encoding="utf-8") as flog:
...
print(f"🗒️  Pistas nuevas usadas guardadas en: {USED_LIST_3}")
