...
# === Verifica que el modelo esté en local
if not os.path.isdir(MODEL_DIR):
    print(f"❌ Error: el modelo no se encuentra en la ruta local '{MODEL_DIR}'.")
    sys.exit(1)
...
model = Wav2Vec2ForCTC.from_pretrained(MODEL_DIR, local_files_only=True).to(device).eval()
...
if not os.path.exists(TSV_PATH):
    print(f"❌ Error: no se encontró el archivo TSV en '{TSV_PATH}'")
    sys.exit(1)
