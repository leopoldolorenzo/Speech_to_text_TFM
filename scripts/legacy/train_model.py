# ✅ scripts/train_model.py (v1 fine-tuning español - optimizado para GPU 12GB)
...
print("[📂] Cargando dataset reducido (máx. 30k ejemplos)...")
dataset = load_from_disk(DATASET_PATH)

print("[🔤] Cargando processor personalizado desde:", TOKENIZER_DIR)
...
training_args = TrainingArguments(
    ...
    group_by_length=False,                     # importante evitar errores de longitud
    per_device_train_batch_size=4,             # adaptado a 12GB GPU
    per_device_eval_batch_size=4,
    gradient_accumulation_steps=8,
    evaluation_strategy="steps",
    eval_steps=500,                            # ✅ evaluación más frecuente
    save_strategy="steps",
    save_steps=500,                            # ✅ checkpoints más frecuentes
    save_total_limit=5,                        # puedes ajustar este límite
    learning_rate=3e-4,
    warmup_steps=500,
    num_train_epochs=10,
    fp16=False,                                # estabilidad
    logging_dir="logs",
    logging_steps=50,
...
# ✅ Reafirmamos group_by_length en Trainer (evita errores al reanudar)
