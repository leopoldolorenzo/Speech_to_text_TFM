...
BASE_MODEL = "models/fine_tune_03"
TOKENIZER_DIR = "models/tokenizer_v1_nospecial"
DATASET_PATH = "data/common_voice_es/es/fine_tune_04/dataset_hf"
OUTPUT_DIR = "training/fine_tune_04"
FINAL_MODEL_DIR = "models/fine_tune_04"
LOGGING_DIR = "logs/fine_tune_04"
...
# === Recuperar último checkpoint si existe ===
...
# === Limpieza de texto de transcripciones ===
def clean_text(text):
    text = text.lower().strip()
    text = re.sub(r"\s+", " ", text)
    return re.sub(r"[^a-záéíóúñü ]+", "", text)

# === Procesamiento de cada muestra (audio + texto) ===
def prepare(batch):
    try:
...
# === Logger personalizado para consola ===
...
# === Cargar dataset HuggingFace desde disco ===
...
# === Revisar checkpoint previo o limpiar ===
...
# === Cargar modelo base y configurar ===
...
# === Preparar colacionador de batches ===
data_collator = PreparadorBatchesCTC(processor=processor, padding=True)

# === Argumentos del Trainer (con límite explícito de pasos) ===
training_args = TrainingArguments(
    ...
    max_steps=total_steps,  # ⬅️ Límite explícito
    learning_rate=3e-5,
...
log.info("🚀 Iniciando entrenamiento incremental (fine_tune_04)...")
